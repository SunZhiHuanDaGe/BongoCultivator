...
4. **视觉与特效 (Visuals)**:
    - **状态与素材映射**:
        - **`cultivator_idle.png` (摸鱼)**: KB < 30 & Mouse < 30.
        - **`cultivator_walk.png` (历练)**: KB >= 30. (键盘为主)
        - **`cultivator_read.png` (悟道)**: Mouse >= 30. (鼠标为主)
        - **`cultivator_cast.png` (斗法)**: KB >= 30 & Mouse >= 30. (高强度操作)
        - **`cultivator_sleep.png` (睡觉)**: IDLE 持续 > 60秒.
